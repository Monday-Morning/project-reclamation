...
const canMutateComment = async (session, authToken, decodedToken, id, authorID, mid, Comment) => {
  ...
  if ((authorID !== mid || !UserPermission.exists(session, authToken, decodedToken, 'comment.write.self'))
...
module.exports = {
  ...
  countOfComments: async(
    _parent, {id, parentType}, { API: {Comment} }
  ) => {
    try {
  ...
  createComment: async(
    _parent,
    { content, authorID, parentID, parentType },
  ...
  updateCommentAuthor: async(
    _parent,{id, authorID}, {session, authToken, decodedToken, mid, API : {Comment}}
    ) => {
      try {
